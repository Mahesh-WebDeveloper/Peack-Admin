...
import { StatsCard } from '../components/EnquiryPageComponents/StatsCard';
import { EnquiryForm } from '../components/EnquiryPageComponents/EnquiryFormPopup';
import { FiltersDialog } from '../components/EnquiryPageComponents/MoreFiltersDialog';
import { Footer } from '../layouts/CommonLayoutComponents/Footer';
import { EnquiryTableFilterSection } from '../components/EnquiryPageComponents/EnquiryTableFilterSection';
import { EnquiryPageDataTable } from '../components/EnquiryPageComponents/EnquiryPageDataTable';
import { EnquiryPageDataTablePagination } from '../components/EnquiryPageComponents/EnquiryPageDataTablePagination';
    ...
    const handleExportPdf = () => {
        ...
        try {
            ...
            const tableData = Array.from(rows).map(row => {
                ...
                return cells.slice(1, cells.length - 1).map(cell => cell.textContent || cell.innerText);
            });

            // Extract headers excluding first and last columns
            const headers = Array.from(table.querySelectorAll("thead th")).slice(1, -1).map(header => header.textContent || header.innerText);
    ...
    return (
        <ThemeProvider>
            <div className="min-h-screen bg-background">
                {/* Sidebar */}
                <aside
                    className={`fixed inset-y-0 left-0 z-40 hidden lg:block transition-all duration-300 ${sidebarCollapsed ? 'w-20' : 'w-64'}`}
                >
                ...
                <div className={`transition-all duration-300 ${sidebarCollapsed ? 'lg:ml-20' : 'lg:ml-64'}`}>
                    ...
                    <main className="container p-4 md:p-6 lg:p-8 bg-gray-200 dark:bg-background/95">
                        ...
                        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4 mb-8">
                            {stats.map(({ label, value, trend, icon: Icon }, index) => (
                                <StatsCard
                                    key={index}
                                    label={label}
                                    value={value}
                        ...
                        {/* Main Table Section */}
                        <div className="rounded-xl  shadow-lg border dark:border-gray-700 bg-background dark:bg-gray-800">
                    
                            <EnquiryTableFilterSection
                            ...
                            />
                            <EnquiryPageDataTable />
                            <EnquiryPageDataTablePagination />
                        </div>
